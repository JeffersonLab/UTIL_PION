...
void run_HeepCoinYield(Int_t RunNumber = 0, Int_t MaxEvent = 0, Double_t threshold_cut = 5, Int_t pscal = 1)
{
  ...
  ofstream myfile1;
  myfile1.open ("PionHeepCoinYieldVar", fstream::app);
  myfile1 << left << "HeeP" << RunNumber << "   " << pscal << "   ";
  myfile1.close();

  //Begin Scaler Efficiency Calculation  
  if(Hostname.Contains("farm")){
    rootFileNameString = Form("/group/c-kaonlt/USERS/${USER}/hallc_replay_lt/ROOTfiles/Analysis/HeeP/coin_replay_Full_Heep_Coin_%i_%i.root",RunNumber,MaxEvent);
  }
  ...
  ch.SetProof();
  ch.Process("HeepCoinYield.C+",option);
  proof->Close();
