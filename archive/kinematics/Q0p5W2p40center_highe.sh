...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/PionLT/Q0p5W2p40center_highe_Pions.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/pionyield/PlotPionPhysics.C(\"${KINFILE}\", \"Q0p5W2p40center_highe_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/PionLT/Q0p5W2p40center_highe_Pions.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/pionyield/PlotPionPhysics.C(\"${KINFILE}\", \"Q0p5W2p40center_highe_Kaons\")"
else echo "Pion plots already found in - ${UTILPATH}/OUTPUT/Analysis/PionLT/Q0p5W2p40center_highe_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
