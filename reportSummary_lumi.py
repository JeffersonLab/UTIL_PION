...
for line in f:
    ...
    for index, obj in enumerate(sumList) :
        if (sumList[index] in data[0]) :
            if (index == 0) :
                fout.write('\n=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:\n\n')
                fout.write('Pion-LT Report Summary')
                fout.write('\n\n=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:\n\n')
                fout.write("Most recent analysis for run %s was completed on" % (runNo) + data[1] + '\n')
f.close()

filename = './REPORT_OUTPUT/COIN/PRODUCTION/PionLT_replay_coin_production_%s_%s.report' % (runNo, evenNo)
...
objList = ['Run #', 
           ...
           'SHMS_pTRIG2 Pre-triggers', 'SHMS_pTRIG3 Pre-triggers', 'SHMS_pTRIG5 Pre-triggers',
           'SHMS_pTRIG2 Pre-scaled Pre-triggers', 'SHMS_pTRIG3 Pre-scaled Pre-triggers', 'SHMS_pTRIG5 Pre-scaled Pre-triggers',
           'Coin ROC2 pTRIG2 Accepted Triggers', 'Coin ROC2 pTRIG3 Accepted Triggers', 'Coin ROC2 pTRIG5 Accepted Triggers', 'ROC2 OG 6 GeV Electronic Dead Time (100, 150) ']
...
replay_file = './REPORT_OUTPUT/COIN/PRODUCTION/PionLT_output_coin_production_%s_%s.report' % (runNo, evenNo)
f    = open(replay_file)
fout.write('\n\n')
