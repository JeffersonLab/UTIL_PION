...
minbin = 0.0 # minimum bin for selecting neutrons events in missing mass distribution
maxbin = 4 # maximum bin for selecting neutrons events in missing mass distribution
...
Pion_Analysis_Distributions = "%s/%s_%s_fADC_Analysis_Distributions.pdf" % (OUTPATH, runNum, MaxEvent)
...
P_MMpi_pions_uncut = ROOT.TH1D("P_MMpi_pions_uncut", "MIssing Mass (no cuts); MM_{#pi}; Counts", 800, 0.0, 4.0)
...
P_MMpi_pions_cut = ROOT.TH1D("P_MMpi_pions_cut", "Missing Mass (with cuts); MM_{#pi}; Counts", 800, 0.0, 4.0)
...
P_MMpi_pions_cut_prompt = ROOT.TH1D("P_MMpi_pions_cut_prompt", "Missing Mass; MM_{#pi}; Counts", 800, 0.0, 4.0)
...
P_MMpi_pions_cut_randm = ROOT.TH1D("P_MMpi_pions_cut_randm", "Missing Mass; MM_{#pi}; Counts", 800, 0.0, 4.0)

P_MMpi_pions_cut_randm_scaled = ROOT.TH1D("P_MMpi_pions_cut_randm_scaled", "Missing Mass; MM_{#pi}; Counts", 800, 0.0, 4.0)
P_MMpi_pions_cut_randm_sub = ROOT.TH1D("P_MMpi_pions_cut_randm_sub", "Missing Mass Rndm Sub; MM_{#pi}; Counts", 800, 0.0, 4.0)
...
ePiCoinTime_vs_MMpi_pions_uncut = ROOT.TH2D("ePiCoinTime_vs_MMpi_pions_uncut","Electron-Pion CTime vs Missing Mass (no cut); e #pi Coin_Time; MM_{#pi}", 200, -40, 40, 400, 0, 4)
...
P_RFTime_vs_MMpi_pions_uncut = ROOT.TH2D("P_RFTime_vs_MMpi_pions_uncut", "SHMS RFTime vs Missing Mass (no cuts); SHMS_RFTime_Dist; MM_{#pi}", 100, 0, 4, 400, 0, 4)
...
P_MMpi_vs_beta_pions_uncut = ROOT.TH2D("P_MMpi_vs_beta_pions_uncut", "Missing Mass vs SHMS #beta (no cut); MM_{#pi}; SHMS_#beta", 400, 0, 4, 200, 0, 2)
...
ePiCoinTime_vs_MMpi_pions_cut = ROOT.TH2D("ePiCoinTime_vs_MMpi_pions_cut","Electron-Pion CTime vs Missing Mass (with PID cuts); e #pi Coin_Time; MM_{#pi}", 100, -2, 2, 400, 0, 4)
...
P_RFTime_vs_MMpi_pions_cut = ROOT.TH2D("P_RFTime_vs_MMpi_pions_cut", "SHMS RFTime vs Missing Mass (with cuts); SHMS_RFTime_Dist; MM_{#pi}", 100, 0, 4, 400, 0, 4)
...
P_MMpi_vs_beta_pions_cut = ROOT.TH2D("P_MMpi_vs_beta_pions_cut", "Missing Mass vs SHMS #beta (with cut); MM_{#pi}; SHMS_#beta", 400, 0, 4, 200, 0, 2)

MMpi_vs_ePiCoinTime_pions_cut_prompt = ROOT.TH2D("MMpi_vs_ePiCoinTime_pions_cut_prompt","Missing Mass vs Electron-Pion CTime; MM_{#pi}; e #pi Coin_Time",400, 0, 4, 100, -2, 2)
...
for k in range(0, 6):
     ...
     Arc.DrawArc(0,0,0.942857*(k+1)/(10),0.,360.,"same")
tradius = TGaxis(0,0,0.575,0,0,1.2,10,"-+")
...
NeutronEvt.AddText("e #pi X Events: %i" %(BinIntegral))
NeutronEvt.Draw()
...
LowerPrompt2 = TLine(PromptWindow[0],gPad.GetUymin(),PromptWindow[0],4)
LowerPrompt2.SetLineColor(2)
LowerPrompt2.SetLineWidth(2)
LowerPrompt2.Draw("same")
UpperPrompt2 = TLine(PromptWindow[1],gPad.GetUymin(),PromptWindow[1],4)
UpperPrompt2.SetLineColor(2)
UpperPrompt2.SetLineWidth(2)
UpperPrompt2.Draw("same")
LowerRandomL2 = TLine(RandomWindows[0],gPad.GetUymin(),RandomWindows[0],4)
LowerRandomL2.SetLineColor(8)
LowerRandomL2.SetLineWidth(2)
LowerRandomL2.Draw("same")
UpperRandomL2 = TLine(RandomWindows[1],gPad.GetUymin(),RandomWindows[1],4)
UpperRandomL2.SetLineColor(8)
UpperRandomL2.SetLineWidth(2)
UpperRandomL2.Draw("same")
LowerRandomR2 = TLine(RandomWindows[2],gPad.GetUymin(),RandomWindows[2],4)
LowerRandomR2.SetLineColor(8)
LowerRandomR2.SetLineWidth(2)
LowerRandomR2.Draw("same")
UpperRandomR2 = TLine(RandomWindows[3],gPad.GetUymin(),RandomWindows[3],4)
...
outHistFile = ROOT.TFile.Open("%s/%s_%s_fADC_Output_Data.root" % (OUTPATH, runNum, MaxEvent), "RECREATE")
