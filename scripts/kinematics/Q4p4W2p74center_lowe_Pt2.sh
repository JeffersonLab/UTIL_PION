...
if [ ! -f "${UTILPATH}/OUTPUT/Analysis/PionLT/Q4p4W2p74center_lowe_Pt2_Pions.root" ]; then
    root -b -l -q "${UTILPATH}/scripts/pionyield/PlotPionPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt2_Kaons\")"
elif [ ! -f "${UTILPATH}/OUTPUT/Analysis/PionLT/Q4p4W2p74center_lowe_Pt2_Pions.pdf" ]; then
    root -b -l -q "${UTILPATH}/scripts/pionyield/PlotPionPhysics.C(\"${KINFILE}\", \"Q4p4W2p74center_lowe_Pt2_Kaons\")"
else echo "Pion plots already found in - ${UTILPATH}/OUTPUT/Analysis/PionLT/Q4p4W2p74center_lowe_Pt2_Kaons.root and .pdf - Plotting macro skipped"
fi
exit 0
