...
while IFS='' read -r line || [[ -n "$line" ]]; do
    runNum=$line
    if [ ! -f "${UTILPATH}/OUTPUT/Analysis/${ANATYPE}LT/${runNum}_-1_CTPeak_Data_HeepCoin.root" ]; then
...
if [ $TestingVar == 1 ]; then
	...
	while IFS='' read -r line || [[ -n "$line" ]]; do
	    runNum=$line
	    if [ -f "${UTILPATH}/OUTPUT/Analysis/${ANATYPE}LT/${runNum}_-1_CTPeak_Data_HeepCoin.root" ]; then
		rm "${UTILPATH}/OUTPUT/Analysis/${ANATYPE}LT/${runNum}_-1_CTPeak_Data_HeepCoin.root"
	    fi
	...
	if [[ $prompt2 == "y" || $prompt2 == "Y" || $prompt2 == "yes" || $prompt2 == "Yes" ]]; then
		...
		runNum=$line
		if [ ! -f "${UTILPATH}/OUTPUT/Analysis/${ANATYPE}LT/${runNum}_-1_CTPeak_Data_HeepCoin.root" ]; then
		    python3 $UTILPATH/scripts/CoinTimePeak/src/CoinTimePeak_HeepCoin.py "${ANATYPE}_coin_replay_production" ${runNum} "-1" 
		fi
	    done < "$RunListFile"
