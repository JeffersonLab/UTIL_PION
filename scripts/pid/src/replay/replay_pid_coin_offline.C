void replay_pid_coin_offline(Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/pid_coin_offline_%d_%d.root";
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("UTIL_KAONLT/config/DEF-files/pid_coin.def");
  // Define cuts file
  analyzer->SetCutFile("UTIL_KAONLT/config/DEF-files/pid_coin_cuts.def");  // optional
...
analyzer->PrintReport("UTIL_KAONLT/config/TEMPLATES/pid_coin.template",
		    Form("UTIL_KAONLT/REPORT_OUTPUT/pid_coin_offline_%d_%d.report", RunNumber, MaxEvent)); // optional
