...
linenum = 0  # Initialize line number counter
nWindows = 0  # Initialize nWindows variable

# Read all lines in the cut file
for line in TimingCutf:
    linenum += 1  # Increment line number
    if linenum > 1:  # Skip the first line (header)
        line = line.partition('#')[0]  # Ignore anything after a '#' (comments)
        line = line.rstrip()
        array = line.split(",")  # Split the line into an array by commas
        if int(runNum) in range(int(array[0]), int(array[1]) + 1):  # Check if runNum is in the range
            nWindows = float(array[5])  # Grab the nWindows value
            break  # Exit the loop once the matching range is found

# Close the file after reading
TimingCutf.close()

print("nWindows = %s for random subtraction" % nWindows)
'''
###############################################################################################################################################
...
def coin_pions():
    ...
    COIN_Pion_Data_Header = [
        ...
        "ph_q", "MandelT", "pmiss", "pmiss_x", "pmiss_y", "pmiss_z", "emiss", "Erecoil", "Mrecoil"
    ]
