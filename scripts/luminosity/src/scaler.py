...
def scaler(PS_names, HMS_PS, SHMS_PS, COIN_PS, thres_curr, report_current, runNum, MaxEvent, s_tree):
    ...
    if(int(runNum) > 14777):
        ...
        scalers.update({"curr_corr" : ((charge_sum[bcm_ix]/time_sum[bcm_ix])+0.01)/(charge_sum[bcm_ix]/time_sum[bcm_ix])})
        print("> 12004, Correction: ",((charge_sum[bcm_ix]/time_sum[bcm_ix])+0.01)/(charge_sum[bcm_ix]/time_sum[bcm_ix]))
