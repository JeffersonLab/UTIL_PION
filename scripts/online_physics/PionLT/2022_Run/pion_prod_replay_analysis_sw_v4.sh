...
# 20/05/22 - Version 4 - Removed scaler replay and BCM "calibration" from script
###########################################################################################################################
...
if [ -f "${UTILPATH}/OUTPUT/Analysis/PionLT/${RUNNUMBER}_${MAXEVENTS}_Analysed_Data.root" ]; then
	...
	echo "Reprocessing"
	python3 ${UTILPATH}/scripts/online_physics/PionLT/pion_prod_analysis_sw.py Pion_coin_replay_production ${RUNNUMBER} ${MAXEVENTS} ${TARGET}
    else
...
if [ -f "${UTILPATH}/OUTPUT/Analysis/PionLT/${RUNNUMBER}_${MAXEVENTS}_Output_Data.root" ]; then
	...
	echo "Reprocessing"
	python3 ${UTILPATH}/scripts/online_physics/PionLT/PlotPionPhysics_sw.py Analysed_Data ${RUNNUMBER} ${MAXEVENTS} ${TARGET}
    else
