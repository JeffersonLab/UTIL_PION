void replay_hms_heep (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  // Define cuts file
  analyzer->SetCutFile("UTIL_PION/config/DEF-files/hms_heep_cuts.def");  // optional
  ...
  analyzer->PrintReport("UTIL_PION/config/TEMPLATES/COIN/coin_production.template", Form("UTIL_PION/REPORT_OUTPUT/Analysis/HeeP/Pion_replay_hms_production_%d_%d.report", RunNumber, MaxEvent)); // optional}
}
