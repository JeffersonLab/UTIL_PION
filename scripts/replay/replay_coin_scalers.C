void replay_coin_scalers (Int_t RunNumber = 0, Int_t MaxEvent = 0,Int_t FirstEvent=1) {
  ...
  const char* ROOTFileNamePattern = "UTIL_PION/ROOTfilesPion/coin_replay_scalers_%d_%d.root";
  ...
  analyzer->SetSummaryFile(Form("UTIL_PION/REPORT_OUTPUT/SCALERS/summary_scalers_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/COIN/SCALERS/coinscalers.template",
  			Form("UTIL_PION/REPORT_OUTPUT/SCALERS/replay_coin_scalers_%d_%d.report", RunNumber, MaxEvent));  // optional  
