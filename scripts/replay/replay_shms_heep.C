void replay_shms_heep (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  // Define cuts file
  analyzer->SetCutFile("UTIL_PION/config/DEF-files/tmp/HeePSing_SHMS_Cuts.def");  // optional
  ...
  analyzer->PrintReport("UTIL_PION/config/TEMPLATES/tmp/Online_HeePSing.template", Form("UTIL_PION/REPORT_OUTPUT/Analysis/HeeP/Pion_replay_shms_production_%d_%d.report", RunNumber, MaxEvent)); // optional}
}
