void replay_production_coin_Helicity (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("UTIL_PION/config/DEF-files/coin_production_Helicity.def");
  // Define cuts file
  analyzer->SetCutFile("UTIL_PION/config/DEF-files/Online_Coin_Production_Cuts_Helicity.def");  // optional
  ...
  analyzer->SetSummaryFile(Form("UTIL_PION/REPORT_OUTPUT/summary_production_%d_%d.report", RunNumber, MaxEvent)); // optional
