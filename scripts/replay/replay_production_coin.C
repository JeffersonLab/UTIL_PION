void replay_production_coin (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  // Define cuts file
  analyzer->SetCutFile("UTIL_PION/config/DEF-files/tmp/Online_Coin_Production_Cuts.def");  // optional
  ...
  analyzer->PrintReport("UTIL_PION/config/TEMPLATES/tmp/Online_Coin_Production.template", Form("UTIL_PION/REPORT_OUTPUT/Analysis/PionLT/Pion_replay_coin_production_%d_%d.report", RunNumber, MaxEvent)); // optional}
}
